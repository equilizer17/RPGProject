 ...
 * @author SumRndmDde and GuilhermeSantos
 *
...
(function (_) {
		...
		for (var i = 0; i < lines.length; i++) {
			...
			if (line.match(regexp2)) {
				...
				if (line.match(regexp3)) {
					...
					var json = JsonEx.parse(_.loadCheckToLocalFile());
					var versionList = json.versionList;
					if (versionList != undefined) {
						var length = versionList.length;
						for (var j = 0; j < length; j++) {
							var data = json.updateData[versionList[j]];
							for (var file = 0; file < data.length; file++) {
								fileIsOk(data[file]);
							}
						}
						function fileIsOk(file) {
							if (temp.loc === file.loc) {
								if (file.restore) temp.restore = true;
								temp.download = file.download;
							}
						}
					}
	...
	_.actuallyDoTheUpdatesAndStuff = function () {
		...
		if (ver && limit) {
			...
			for (var j = 0; j < updateData.length; j++) {
				...
				var fs = require('fs');
				var filePath = _.localPath(info.loc);
				if (!fs.existsSync(filePath) && info.restore) {
					info.download = true;
					_.downloadFile(info.url, info.loc, _.updateIndex);
					_.applyCheckUpdates();
					_.updateIndex++;
				} else if (fs.existsSync(filePath) && info.restore) {
					info.download = true;
					_.downloadFile(info.url, info.loc, _.updateIndex);
					_.applyCheckUpdates();
					_.updateIndex++;
				} else if (info.download === undefined) {
					info.download = true;
					_.downloadFile(info.url, info.loc, _.updateIndex);
					_.applyCheckUpdates();
					_.updateIndex++;
	...
	_.localFilePathCheck = function () {
		return StorageManager.localFileDirectoryPath() + 'check.sumrndmdde';
	};

	_.applyCheckUpdates = function () {
		var json = { "updateData": _.updateData, "versionList": _.versionList }
		var data = LZString.compressToBase64(JsonEx.stringify(json));
		var fs = require('fs');
		var dirPath = StorageManager.localFileDirectoryPath();
		var filePath = _.localFilePathCheck();
		if (!fs.existsSync(dirPath)) {
			fs.mkdirSync(dirPath);
		}
		fs.writeFileSync(filePath, data);
	...
	_.loadCheckToLocalFile = function () {
		var data = null;
		var fs = require('fs');
		var filePath = _.localFilePathCheck();
		if (!fs.existsSync(filePath)) {
	...
	_.verifyCheckUpdates = function () {
		...
		function fileIsOk(file) {
			var fs = require('fs');
			var filePath = _.localPath(file.loc);
			if (!file.download && !fs.existsSync(filePath)) {
				fs.unlink(_.localPath('save/version.sumrndmdde'));
				fs.unlink(_.localPath('save/updates.sumrndmdde'));
				file.restore = true;
				var data = LZString.compressToBase64(JsonEx.stringify(json));
				var fs = require('fs');
				var dirPath = StorageManager.localFileDirectoryPath();
			...
			if (fs.existsSync(filePath)) {
				...
				if (file.size != fileSizeInMegabytes) {
					fs.unlink(_.localPath(filePath));
					fs.unlink(_.localPath('save/version.sumrndmdde'));
					fs.unlink(_.localPath('save/updates.sumrndmdde'));
					file.restore = true;
					var data = LZString.compressToBase64(JsonEx.stringify(json));
